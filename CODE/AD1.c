...
static word AD1_OutV[3];               /* Array of measured values */
/*
** ===================================================================
...
ISR(AD1_Interrupt)
{
  AD1_OutV[SumChan] += ATDDR0L;        /* Save measured value */
  ...
  if (SumChan == 3U) {                 /* Is number of measurement equal to the number of channels? */
    ...
    SumCnt++;
    if (SumCnt == 8U) {                /* Is number of measurement on each channel equal to the number of conversions? */
...
byte AD1_Measure(bool WaitForResult)
{
  ...
  SumCnt = 0U;                         /* Clear the counter of conversions */
...
byte AD1_GetValue(void *Values)
{
  ...
  /* Note: Next 3 lines are speed optimized */
  *((byte*)Values)++ = (byte)(AD1_OutV[0] / 8U); /* Save measured values to the output buffer */
  *((byte*)Values)++ = (byte)(AD1_OutV[1] / 8U); /* Save measured values to the output buffer */
  *((byte*)Values) = (byte)(AD1_OutV[2] / 8U); /* Save measured values to the output buffer */
  return ERR_OK;                       /* OK */
}
...
byte AD1_GetValue16(word *Values)
{
  ...
  /* Note: Next 3 lines are speed optimized */
  *Values++ = AD1_OutV[0] << 5U;       /* Save measured values to the output buffer */
  *Values++ = AD1_OutV[1] << 5U;       /* Save measured values to the output buffer */
  *Values = AD1_OutV[2] << 5U;         /* Save measured values to the output buffer */
  return ERR_OK;                       /* OK */
}
